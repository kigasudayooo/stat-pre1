...
def _(mo):
    mo.md(
        r"""
        主成分分析の目的は、データの分散を最大化する方向を見つけること。単位ベクトル $w$ （$|w|=1$）への射影を考えると、その射影データの分散は：

        $$\text{Var}(X_Cw) = \frac{1}{n-1}(X_Cw)^T(X_Cw) = \frac{1}{n-1}w^T X_C^T X_C w = w^T S w$$

        ここで $S = \frac{1}{n-1}X_C^T X_C$ はデータの共分散行列。

        ## データの方向への射影

        「データを方向 $w$ に射影する」とは、多次元データを特定の方向に沿って一次元の値に変換すること。
...
def _(mo):
    mo.md(
        ...
        共分散行列のトレース（対角成分の和）は元データの総分散を表し、固有値の和と一致します：

        $$\text{tr}(S) = \sum_{i} \text{Var}(X_i) = \sum_{i} \lambda_i$$

        固有値が大きいほど、対応する固有ベクトル方向のデータの分散が大きいことを意味します。つまり、固有値の大きい順に固有ベクトルを選ぶことで、データの情報をできるだけ保持しながら次元を削減できます。

        最大の固有値 $\lambda_1$ に対応する固有ベクトル $p_1$ が第一主成分、2番目に大きい固有値 $\lambda_2$ に対応する固有ベクトル $p_2$ が第二主成分というように続きます。
        """
    )
    return
...
def _(mo):
    mo.md(
        ...
        特異値分解はより正方行列ではない行列にも適用できる。$m \times n$ 行列 $A$ の特異値分解は：
        ...
        行列 $A^T A$ の固有ベクトルは、$A$ の右特異ベクトル $V$ と一致する
        $A A^T$ の固有ベクトルは、$A$ の左特異ベクトル $U$ と一致する
        $A^T A$ および $A A^T$ の固有値は $\sigma_i^2$ となる（ここで $\sigma_i$ は $A$ の特異値）
